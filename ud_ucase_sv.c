...
main(int argc, char *argv[])
{
    ...
    socklen_t len;
    char buf[BUF_SIZE];
    struct bcm2835_peripheral gpio = {GPIO_BASE}; //GPIO SETUP
    ...
    for (;;) {
        ...
        //toggelen GPIO
          for (int i = 0; i < 5; i++) //5 KEER
        {
            GPIO_SET = 1 << received_data.IO; //toggel on
            sleep(received_data.period);      //w8
            GPIO_CLR = 1 << received_data.IO; //toggel off
            sleep(received_data.period);      //w8
        }
        
        //laten staan sendto code
